...
OverallRecord<-(.4125) #The team's overall ABILITY to win. This is the proportion assumed by the
#null hypothesis, that is, the 2004-2008 Cardinals had the same ABILITY to win at home and
#on the road. The Cardinals won 33 of 80 overall, giving us the null hypothesis proportion of 33/80 = .4125 

Category1<-40 #Total number of results in the first category (e.g., home games, clay surface, etc.) 
#In the Cardinals example, we put 40 to represent the number of home games played.

Category2<-40 #Total number of results for the second category (e.g., aways games, other surfaces, etc.)

#We will run 1000 simulations. (e.g., 1000 simulation for the 40 home and 40 away games played between 2004 and 2008.

NumOfSims<-1000 #(See note on line 93 regarding the number of simulations and dot plot graphics.
...
#in winning percentage is calculated. This process is repeated NumOfSims times, and NumOfSims dots (e.g., 1000)
#are recorded on the dotplot.

#CATEGORY 1 SIMULATION (e.g., home games)
...
{(RandList1<-sample(1:10000, Category1, replace=FALSE)) #Generates random integers between 1 and 10000.
  #This is done NumOfAttempts times, and the integers are saved in the RandList matrix.
  ...
  # We rename these matricies Outcomes1 and Outcomes 2.
  Outcomes1<-AssignFailure1
  Outcomes2<-AssignFailure2
  
  #By adding the elements of the Outcomes matrices, we obtain the simulated total number successes.
  ...
  #We now convert the total number of wins to a proportion. In our example, this 
...
#(e.g. the simulated difference in home and away performances in 80 total games played, assuming the
#Cardinals have the same ability to win at home and on the road).
...
#first edition of the Statistical Reasoning of Sports), we use use R's built-in strip chart:
